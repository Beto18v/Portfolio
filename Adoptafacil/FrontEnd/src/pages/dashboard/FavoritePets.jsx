...
export default function FavoritePets() {
  // Datos de ejemplo para mascotas favoritas
  const favorites = [
    {
      id: 1,
      name: 'Max',
  ...
  return (
    <div className="flex-auto md:px-52 p-8 fade-in bg-gradient-to-r from-green-400 to-blue-500 dark:from-green-600 dark:to-blue-700">
      <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Mascotas Favoritas</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {favorites.map((pet) => (
          <div key={pet.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            <div className="relative h-40">
              ...
              <div className="absolute top-2 right-2">
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${pet.status === 'Disponible' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300'}`}>
                  {pet.status}
                </span>
              </div>
            </div>
